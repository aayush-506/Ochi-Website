...
const Navbar = () => {
  return (
    <div className=' Navbar fixed z-[9999] font-[Neue_Montreal] w-full  px-13 py-5 flex justify-between items-center '>
      <div className='logo'>
      ...
      <div className='flex gap-2'>
        {["Services", "Our Work", "About us", "Insights", "Contact us"].map((items, index) => (
          <a key={index} className={`relative w-20 h-10 overflow-hidden Capitalize font-light cursor-pointer group ${index == 4 && 'ml-62'}`}>

            <span className="font-[Neue_montreal] text-md absolute w-full h-full flex items-center justify-center transition-transform duration-500 group-hover:-translate-y-full">
              {items}
            </span>

            <span className="font-[Neue_montreal] text-md absolute w-full h-full top-full flex items-center justify-center transition-transform duration-500 group-hover:-translate-y-full">
              {items}
            </span>
            <span className="absolute left-0 bottom-1 w-0 h-[1px] bg-black transition-all duration-500 mt-[-1vh] group-hover:w-full"></span>
